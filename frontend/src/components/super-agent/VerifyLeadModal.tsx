...
export const VerifyLeadModal: React.FC<Props> = ({ lead, isOpen, onClose, onSuccess }) => {
    ...
    const mutation = useMutation({
        mutationFn: () => leadsApi.verifySALead(lead.ulid, notes || undefined),
        onSuccess: () => {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-auto overflow-hidden max-h-[90vh] flex flex-col">
                <div className="px-6 py-4 bg-gradient-to-r from-green-600 to-emerald-600 text-white flex-shrink-0">
                    <h3 className="text-lg font-semibold">✅ Verify Lead</h3>
                    <p className="text-green-200 text-sm mt-0.5">This will send the lead to Admin for processing</p>
                </div>
                ...
                <div className="overflow-y-auto flex-1 p-6 space-y-4">
                    <div className="p-4 bg-gray-50 rounded-xl space-y-1">
                        ...
                        {lead.submitted_by_agent && (
                            <p className="text-sm text-gray-500">Submitted by: <span className="text-gray-700">{lead.submitted_by_agent.name} ({lead.submitted_by_agent.agent_id})</span></p>
                        )}
                ...
                <div className="px-6 pb-6 pt-3 flex gap-3 justify-end border-t border-gray-100 flex-shrink-0">
                    ...
                    >
                        {mutation.isPending ? 'Verifying…' : '✅ Verify & Send to Admin'}
                    </button>
                </div>
            </div>
        </div>
