...
export const RevertLeadModal: React.FC<Props> = ({ lead, isOpen, onClose, onSuccess }) => {
    ...
    const mutation = useMutation({
        mutationFn: () => leadsApi.revertAgentLead(lead.ulid, reason),
        onSuccess: () => {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden">
                <div className="px-6 py-4 bg-gradient-to-r from-red-500 to-rose-600 text-white">
                    <h3 className="text-lg font-semibold">↩ Revert Lead to Enumerator</h3>
                    <p className="text-red-100 text-sm mt-0.5">Enumerator will be asked to correct and resubmit</p>
                </div>
                <div className="p-6 space-y-4">
                    <div className="p-4 bg-gray-50 rounded-xl space-y-1">
                        ...
                        {lead.submitted_by_enumerator && (
                            <p className="text-sm text-gray-500">Enumerator: <span className="text-gray-700">{lead.submitted_by_enumerator.name}</span></p>
                        )}
                    ...
                    {isLastRevert && (
                        <div className="flex items-start gap-2 p-3 bg-red-50 border border-red-200 rounded-xl">
                            ...
                            <p className="text-red-700 text-sm font-medium">
                                This is the 3rd revert. The lead will be <strong>automatically escalated to Admin</strong> after this action — the enumerator will not be given another chance to correct it.
                            </p>
                        </div>
                    )}
                    ...
                    <div>
                        ...
                        />
                        <p className="text-xs text-gray-400 mt-1">Minimum 10 characters. This will be shown to the enumerator.</p>
                    </div>
                    {mutation.isError && (
                ...
                <div className="px-6 pb-6 flex gap-3 justify-end">
                    ...
                    >
                        {mutation.isPending ? 'Reverting…' : '↩ Revert to Enumerator'}
                    </button>
                </div>
            </div>
        </div>
